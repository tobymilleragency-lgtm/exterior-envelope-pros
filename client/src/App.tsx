...
function Hero({ eyebrow, title, body, visual = true }: { eyebrow: string; title: string; body: string; visual?: boolean }) {
  return <section className="relative isolate overflow-hidden bg-[oklch(0.18_0.035_155)] px-6 py-16 text-white sm:py-24 lg:py-28"><div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_78%_18%,oklch(0.72_0.16_82/.38),transparent_34%),linear-gradient(135deg,oklch(0.16_0.035_155),oklch(0.24_0.055_128)_48%,oklch(0.12_0.03_238))]" /><div className="absolute inset-x-0 bottom-0 -z-10 h-40 bg-[linear-gradient(to_top,oklch(0.08_0.02_155/.72),transparent)]" /><div className="container grid min-h-[620px] gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-center"><div><Badge className="mb-5 border-white/20 bg-white/10 px-4 py-2 text-sm font-black text-white shadow-lg backdrop-blur">{eyebrow}</Badge><h1 className="max-w-5xl text-5xl font-black leading-[0.95] tracking-[-0.06em] text-white sm:text-7xl lg:text-8xl">{title}</h1><p className="mt-7 max-w-2xl text-lg leading-8 text-white/82 sm:text-xl">{body}</p><div className="mt-9 flex flex-col gap-3 sm:flex-row"><AppLink href={business.phoneHref}><Button size="lg" className="h-14 bg-white px-7 font-black text-slate-950 shadow-xl hover:bg-white/90"><Phone className="mr-2 h-5 w-5" /> Call {business.phone}</Button></AppLink><AppLink href="/contact"><Button size="lg" variant="outline" className="h-14 border-white/35 bg-white/10 px-7 font-black text-white backdrop-blur hover:bg-white/20">Free Exterior Estimate <ArrowRight className="ml-2 h-5 w-5" /></Button></AppLink></div><div className="mt-10 grid max-w-2xl gap-3 text-sm font-bold text-white/80 sm:grid-cols-3"><span className="rounded-full border border-white/15 bg-white/10 px-4 py-3 backdrop-blur">Siding + trim</span><span className="rounded-full border border-white/15 bg-white/10 px-4 py-3 backdrop-blur">Gutters + drainage</span><span className="rounded-full border border-white/15 bg-white/10 px-4 py-3 backdrop-blur">Storm documentation</span></div></div>{visual && <LeadOpsVisual />}</div></section>;
}

function LeadOpsVisual() {
  return <div className="relative mx-auto w-full max-w-xl"><div className="absolute -inset-6 rounded-[3rem] bg-white/10 blur-2xl" /><div className="relative overflow-hidden rounded-[2.25rem] border border-white/20 bg-white/12 p-4 shadow-2xl backdrop-blur-xl"><div className="overflow-hidden rounded-[1.75rem] bg-stone-100 text-slate-950 shadow-2xl"><div className="relative h-72 bg-[linear-gradient(180deg,oklch(0.86_0.02_90),oklch(0.7_0.03_85))]"><div className="absolute inset-x-10 top-10 h-16 skew-x-[-18deg] rounded-t-xl bg-slate-900 shadow-2xl" /><div className="absolute bottom-0 left-12 right-12 h-48 rounded-t-2xl bg-[linear-gradient(90deg,oklch(0.74_0.03_85),oklch(0.92_0.018_80))] shadow-2xl"><div className="absolute left-8 top-8 h-24 w-16 rounded-t-xl bg-slate-800" /><div className="absolute right-8 top-8 grid grid-cols-2 gap-3">{[0,1,2,3].map((item) => <div key={item} className="h-12 w-16 rounded-lg border-4 border-white bg-sky-200 shadow" />)}</div></div><div className="absolute bottom-0 left-0 right-0 h-10 bg-[linear-gradient(90deg,oklch(0.38_0.08_142),oklch(0.55_0.11_140))]" /></div><div className="grid gap-3 p-5">{["Whole-home exterior plan", "Material + color board", "Drainage and storm checklist"].map((item) => <div key={item} className="flex items-center gap-3 rounded-xl bg-slate-100 p-3 text-sm font-black"><CheckCircle2 className="h-4 w-4 text-emerald-700" />{item}</div>)}</div></div></div></div>;
}
...
function HomePage() { return <><Hero eyebrow="Siding • Gutters • Windows • Doors" title="One exterior plan. No mismatched pieces." body="Prairie Exterior Co. handles siding, gutters, windows, doors, soffit, fascia, trim, and storm restoration as one coordinated outside package." /><ServiceGrid /><LeadFlowLineSection /><LeadLeakAudit /><Showcase /><Trust /><Insurance /><BeforeAfterComparison /><Reviews /><ServiceAreaSummary /><CTA /></>; }
